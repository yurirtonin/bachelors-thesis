...
img = Image.open('image1.png').convert('L') #Converting to grayscale
...
flipped_image = np.fliplr(imagem)
plb.imshow(flipped_image,cmap='gray')
rotated_image = np.rot90(flipped_image,1)
plb.imshow(rotated_image,cmap='gray')
plt.show()

#Fourier transform
FT = np.fft.fft2(rotated_image)
...
def plotting(image1,image2,label1,label2,filename):
    plot_rotate_back = np.rot90(image1,3)
    plot_flip_back = np.fliplr(plot_rotate_back)
    image1 = plot_flip_back
    
    plot_rotate_back = np.rot90(image2,3)
    plot_flip_back = np.fliplr(plot_rotate_back)
    image2 = plot_flip_back
...
plot_rotate_back = np.rot90(IFT_filtering1_plot,3)
plot_flip_back = np.fliplr(plot_rotate_back)
IFT_filtering1_plot = plot_flip_back

plot_rotate_back = np.rot90(IFT_filtering2_plot,3)
plot_flip_back = np.fliplr(plot_rotate_back)
IFT_filtering2_plot = plot_flip_back
