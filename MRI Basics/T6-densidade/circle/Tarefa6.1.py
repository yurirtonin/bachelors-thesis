...
for m in range(0,13):
    ...
    for i in range(0,100):
        if(Radius**2 > position_array[np.int(n/2-i)]**2):
            density_array[np.int(n/2-i)] = np.sqrt(Radius**2-position_array[np.int(n/2-i)]**2)
            density_array[np.int(n/2+i)] = np.sqrt(Radius**2-position_array[np.int(n/2+i)]**2)
