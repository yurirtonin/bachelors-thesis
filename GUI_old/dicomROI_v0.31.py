...
# Create objects (create widgets) #
        ...
        self.slice_number_scale = Scales(self.left_frame,"Slice Number",1,24)
        self.echo_time_scale    = Scales(self.left_frame,"Echo Time",0,5)         # echos = np.array(np.unique(slice_and_echo[:, 1]))  # Create array removing repeated values from echo array and reordering them
