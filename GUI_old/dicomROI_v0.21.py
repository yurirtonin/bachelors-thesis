...
class Program():
    ...
    def roi_average(self):

        x_init.cget("text")
        print(x_init.cget("text"))

        for i in range(self.dcm_all_echoes):
            dcm_image = Image.fromarray(self.dcm_all_echoes[i])
...
place_search_folder = SearchFolder(top_frame)

place_scales = Scales(left_frame)

place_interactive_canvas = InteractiveCanvas(left_frame)
place_interactive_canvas.pack()


save_ROI = Button(left_frame,text='Obter ROI',command=place_interactive_canvas.get_ROI)
save_ROI.pack(side=LEFT)

x_init = Label(left_frame,text=' x0 = ',relief= RIDGE)
x_init.pack(side=LEFT)

# average_button = Button(left_frame,text='Obter ROI',command=.roi_average())
# average_button.pack(side=RIGHT)

# graph_file = ImageTk.PhotoImage(file="greyscale.png")
# graph = Label(right_frame, image=graph_file)
# graph.pack(anchor="s")

Scales().slice_number.trace("w", Program().change_image)  # Goes to function change_image when slicer is changed
Scales().echo_time.trace("w", Program().change_image)     # Goes to function change_image when slicer is changed
