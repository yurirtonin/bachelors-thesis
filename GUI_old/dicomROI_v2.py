...
class Plot():
    ...
    def __init__(self,slice_and_echo,first_plot,right_frame):
        ...
        self.amplitude = Parameter(0.0001)
        self.decay_const = Parameter(0.0001)
    ...
    def fit(self,function, parameters, y, x=None):
        ...
        def f(params):
            ...
            for p in parameters:
                p.set(params[i])
                # print('type params[i] : {0:s}'.format(type(params[i]).__name__))
                i += 1
            return y - function(x)
    ...
    def create_plot(self):
        ...
        self.fit_parameters, self.success = self.fit(self.function,[self.amplitude,self.decay_const],self.average)
